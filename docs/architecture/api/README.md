...
| **SHOULD** only use UUIDs if necessary | ✅ | Using cuid v1 via Prisma `@default(cuid())` (25 chars). cuid v1 is shorter than UUID and collision-resistant. Note: cuid(2) was the original choice but prisma-client-python 0.15 bundles a Prisma 5.x engine that can't parse cuid(2); reverting to cuid() until prisma-client-python supports Prisma 6+. All ID columns are `String`, not `number`, preserving flexibility per the guideline. |
